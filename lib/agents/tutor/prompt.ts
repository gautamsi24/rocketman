...
export function buildTutorInstructions(context: TutorContext): string {
  ...
  return [
    ...
    "- Mermaid syntax must be strictly valid or the diagram fails to render at all. The most common break: an unquoted subgraph or node label containing spaces, parentheses, or punctuation -- write `subgraph SG1[\"Extracellular Fluid (Watery)\"]`, never `subgraph Extracellular Fluid (Watery)`. Quote every label that isn't a single plain word, keep node ids short and alphanumeric (e.g. `H1`, `T`), and double-check the diagram would actually parse before emitting it.",
    "",
